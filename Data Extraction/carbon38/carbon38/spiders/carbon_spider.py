...
class Main(scrapy.Spider):
    ...
    def parse(self, response):
        for product in response.css('div.ProductItem__Wrapper'):
            yield {
                'name': product.css('h3.ProductItem__Designer::text').get(default='N/A'),
                'price': product.css('div.ProductItem_PriceList span.ProductItem_Price::text').get(default='sold out'),
                'image': product.css('img.ProductItem__Image--alternate::attr(src)').get(),
                # 'url': product.css('a.ProductItem__ImageWrapper::attr(href)').get()
                }